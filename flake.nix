        ...
        [
...

          {
            nixpkgs.overlays = mkBefore [
              (final: prev: {
                inherit (final._channels.nixos-22_05)
                  # Broken builds on `nixpkgs`
                  ark buku krita
                  ;

                inherit (final._channels.nixos-22_05-small)
                  # "Expensive" builds
                ...
                _channels = genAttrs
                  [ "nixos-22_05" "nixos-22_05-small" "nixos-unstable" "nixos-unstable-small" "nixpkgs-unstable" "nixpkgs" ]
