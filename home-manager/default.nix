...
rec {
  imports =
    [
      ...
      # flakes.hyprland.homeManagerModules.default
      {
        ...
        # home.sessionVariables.GDK_DPI_SCALE = 1.6;
        # Doubles scale with GDK_DPI_SCALE on Thunderbird
        ...
        home.sessionVariables.WLR_DRM_FORCE_LIBLIFTOFF = 1;
        home.sessionVariables.WLR_RENDERER_ALLOW_SOFTWARE = 1;
        # Prefer integrated GPU
        # udevadm info -a -n /dev/dri/card1 | grep boot_vga
...
exec = ${pkgs.eww}/bin/eww --restart --config ~/Projects/nixos-configuration/home-manager/eww open-many side-bar
...
'';
  ...
  # Needed for major assumptions from other applications
